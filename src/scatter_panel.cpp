#include <graph_rviz_plugin/scatter_panel.hpp>
...
{
  connect(this, &ScatterPanel::enable, this, &ScatterPanel::setEnabled);
  setName("Scatter graph");
  ...
  connect(this, &ScatterPanel::displayMessageBox, this, &ScatterPanel::displayMessageBoxHandler);

  connect(start_pause_button_, &QPushButton::clicked, this, &ScatterPanel::startPauseClicked);
  connect(stop_button_, &QPushButton::clicked, this, &ScatterPanel::stopClicked);
  connect(topic_button_, &QPushButton::clicked, this, &ScatterPanel::topicsSelectionClicked);
  connect(graph_settings_button_, &QPushButton::clicked, this, &ScatterPanel::graphSettingsClicked);
  QPushButton *settings_button = new QPushButton("Settings");
  connect(settings_button, &QPushButton::clicked, this, &ScatterPanel::settingsClicked);
  connect(export_button_, &QPushButton::clicked, this, &ScatterPanel::exportClicked);
  QPushButton *reset_button = new QPushButton("Reset");
  connect(reset_button, &QPushButton::clicked, this, &ScatterPanel::resetClicked);
  ...
  connect(graph_refresh_timer_, &QTimer::timeout, this, &ScatterPanel::graphUpdate);
...
void ScatterPanel::graphSettingsUpdate()
{
  for (unsigned i = 0; i < displayed_topics_.size(); i++)
  {
    ...
    plot_->graph(i)->setScatterStyle(displayed_topics_.at(i)->scatter_shape_);
...
void ScatterPanel::graphUpdate()
{
  for (unsigned i = 0; i < displayed_topics_.size(); i++)
  {
    if (displayed_topics_.at(i)->data_update_ == true)
    {
      QVector<QVector<double> > topic_data = displayed_topics_.at(i)->getTopicScatterData();
      ...
      //Remove time as key, instead use x,y from topic
      QVector<double> x;
      QVector<double> y;
      for (int i=0; i<topic_data.size(); i++)
      {
        QVector<double> vtr = topic_data.at(i);
        x.push_back(vtr.at(0));
        y.push_back(vtr.at(1));
      }
      plot_->graph(i)->setData(x, y, true);
...
void ScatterPanel::displayMessageBoxHandler(const QString title,
...
void ScatterPanel::load(const rviz::Config &config)
{
...
void ScatterPanel::save(rviz::Config config) const
{
...
void ScatterPanel::startPauseClicked()
{
...
void ScatterPanel::resetClicked()
{
  if ((graph_refresh_timer_->isActive()) == true)
    startPauseClicked();
...
void ScatterPanel::exportClicked()
{
...
void ScatterPanel::topicsSelectionClicked()
{
  ...
  allowed_topics.emplace_back("geometry_msgs/Point");
...
void ScatterPanel::graphSettingsClicked()
{
...
void ScatterPanel::settingsClicked()
{
...
void ScatterPanel::enableLegend(bool legend_enable)
{
...
PLUGINLIB_EXPORT_CLASS(graph_rviz_plugin::ScatterPanel, rviz::Panel)
